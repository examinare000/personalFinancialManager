...
4. **既存資産**: `ui/Ph3/01` の基盤、`shared/kakeibo_shared/domain/holding.py` の `SymbolKind` 列挙（Phase 1.2）
...
3. **`symbol_kind` の表記**: `shared/kakeibo_shared/domain/holding.py` の `SymbolKind`（`stock` / `fund` / `etf` / `crypto` / `cash`）と整合。日本語表示は `stock` → `株式` のような変換を `ui/lib/portfolio/labels.ts` に集約。
...
- **`symbol_kind` の文字列定義**: `shared/kakeibo_shared/domain/holding.py` と一致させる（Coder は当該ファイルを Read）。
