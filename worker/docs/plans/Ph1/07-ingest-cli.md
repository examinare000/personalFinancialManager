...
`python -m kakeibo_worker.ingest <institution> <path>` 相当のコマンドを提供し、**アダプタ起動 → DB 永続化 → 原本退避**までを一気通貫で実行する CLI を実装する。Phase1 完了条件 (b)「同一 CSV を 2 回投入しても `transactions` 行数が増えない」を直接満たす。
...
- CLI エントリポイント `worker/src/kakeibo_worker/ingest/cli.py`。`click` または `argparse` ベース。
...
| パス | 役割 |
|---|---|
| `worker/src/kakeibo_worker/ingest/__init__.py` | パッケージ初期化 |
...
2. **コマンドシグネチャ**: `kakeibo_worker.ingest <institution> <path> [--dry-run] [--account-id INT]`。`--account-id` は CSV に口座 ID が含まれない場合の指定用（Phase1 では機関＋口座が 1:1 を仮定）。
