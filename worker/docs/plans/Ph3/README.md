...
| 範囲 | パス | Phase 3 での扱い |
|---|---|---|
| 取込CLI | `worker/src/kakeibo_worker/ingest/cli.py` | 変更なし |
| Watcher / Archiver | `worker/src/kakeibo_worker/ingest/{watcher,dispatch,archiver}.py` | 変更なし |
| メールパーサ群 | `worker/src/kakeibo_worker/adapters/mail/` | 変更なし |
| PayPal API クライアント | `worker/src/kakeibo_worker/adapters/paypal.py` | 変更なし |
| Scheduler / Jobs | `worker/src/kakeibo_worker/{scheduler,jobs/}.py` | 変更なし |
...
これらは Phase 4 着手時に `worker/docs/plans/Ph4/` を新設して扱う。
