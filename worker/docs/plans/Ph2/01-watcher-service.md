...
`worker` コンテナ。Phase 0 で実装した heartbeat ループ（`worker/src/kakeibo_worker/main.py`）を **取込スケジューラのディスパッチャに発展させる**（`worker/src/kakeibo_worker/__init__.py` のコメント参照）。`/inbox/<institution>/` 配下の新規ファイルを監視し、Phase 1.6 で実装した取込CLIを subprocess（or in-process 関数）で起動する。
...
5. **既存資産**:
   - `worker/src/kakeibo_worker/main.py`（Phase 0 heartbeat ループ。本タスクで内容拡張）
   - `worker/src/kakeibo_worker/ingest/cli.py`（Phase 1.6 で実装される取込CLI）
   - `worker/src/kakeibo_worker/ingest/registry.py`（機関 → アダプタ）
...
3. **取込起動方式**: 当面は **in-process 関数呼び出し**（`from kakeibo_worker.ingest.cli import ingest_file; ingest_file(institution, path)`）を採用。subprocess は cron バッチ（`worker/Ph2/08`）と Phase 2.1 のリトライで利用検討。
...
- `worker/src/kakeibo_worker/ingest/dispatch.py` を最小実装（機関コード判定 + `ingest_file` 呼び出し）。
- `worker/src/kakeibo_worker/ingest/watcher.py` で `Observer` を初期化し、`FileSystemEventHandler` のサブクラスを登録。
- `worker/src/kakeibo_worker/main.py` の `run()` を改修し、`heartbeat_path` ループと並行して Watcher を起動。
