...
| `shared/kakeibo_shared/config.py` 改修 | 慎重に | `Settings` 既存フィールド（`paypal_api_secret`, `gmail_oauth_token_path`）を **API 側が import しても壊れない** ことが必要 |
...
| Worker | 役割 |
|---|---|
| Reviewer | `worker/Ph2/*` のタスクで `pyproject.toml` / `shared/kakeibo_shared/__init__.py` / `shared/kakeibo_shared/config.py` が変更された際、`api` コンテナのビルド・起動と既存テスト（`tests/unit/test_settings_repr.py` など）への影響をチェック |
