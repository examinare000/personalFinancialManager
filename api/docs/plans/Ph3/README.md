...
| 範囲 | パス |
|---|---|
| Flask アプリファクトリ拡張 | `api/src/kakeibo_api/app.py`（既存ファクトリに Blueprint / 認証 / OpenAPI を登録） |
| REST Blueprint 群 | `api/src/kakeibo_api/blueprints/{balances,transactions,categories,rules,aggregations}.py` |
| pydantic v2 リクエスト/レスポンスモデル | `api/src/kakeibo_api/schemas/` |
| 認証ミドルウェア（Tailscale ヘッダ） | `api/src/kakeibo_api/auth.py` |
